...
def clean_df(df):
    df_traj = df['Trajectory']
    df_x = df['x']
    df_y = df['y']
    df_new = pd.concat([df_traj, df_x, df_y], axis=1)
    return df_new
...
def show_trajectory(filename, traj, fpra=1, heading_std_threshold=0.4, speed_fraction=0.66):
    df = clean_df(pd.read_csv(filename, encoding='utf-8'))
...
def heading_roll(x, y, ffpra, traj):
    ...
    for m in range(len(heading_vector[0]) - ffpra):
        x_direction.append((1 / np.pi) * np.arccos(np.average(heading_vector[0][m:m + ffpra]) / np.sqrt(
            np.average(heading_vector[0][m:m + ffpra]) ** 2 + np.average(heading_vector[1][m:m + ffpra]) ** 2)))
        y_direction.append((1 / np.pi) * np.arccos(np.average(heading_vector[1][m:m + ffpra]) / np.sqrt(
            np.average(heading_vector[0][m:m + ffpra]) ** 2 + np.average(heading_vector[1][m:m + ffpra]) ** 2)))
...
def plot_event_history(tumble_length_history):
    ...
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.set_xlabel("Tick Length")
    ax1.set_ylabel("Frequency")
    ax1.hist(tumble_length_history.values(), math.floor(max(tumble_length_history.values()) / 2))
    fig.suptitle("Tumble length n=(" + str(len(tumble_length_history.values())) + ")")
    plt.show()
    return


def get_events(filedir, fpra, frame_threshold_count, heading_std_threshold=0.4, speed_fraction=0.66, write=False,
               debugprints=False):
    ...
    for filename in files:
        ...
        # Read csv file
        df = clean_df(pd.read_csv(file, encoding='utf-8'))
        ...
        for trajectory in x_pos:
            ...
            for k in range(5, len(rolled_speed) - 2):
                ...
                if rolled_speed[k] < speed_fraction * np.median(rolled_speed):
                    ...
                    if event_slowing:
                        slowed = True
                        if tumble_length_history.get(filename + " " + str(trajectory)) is None:
                            tumble_length_history.update({filename + " " + str(trajectory): 1})
                        else:
                            tumble_length_history.update({filename + " " + str(trajectory): tumble_length_history.get(
                ...
                if not directions_swap and not changing_direction and not slowing:
                    if slowed and prolonged_changing_direction and tumble_length_history.get(
                        ...
                        if debugprints:
                            ...
                            debug(event_slowing, event_directions_swap, event_changing_direction, slowed,
                                  prolonged_changing_direction)
                    ...
                    elif event_slowing or event_changing_direction:
                        result = 'reverse'
                        if debugprints:
        ...
        prob_of_rev = reverses / (total_frames / framerate)
        prob_of_tumble = tumbles / (total_frames / framerate)

        if write:
            f.write("""
            """ + str(filename[0:-4]) + "," + str(fpra) + "," + str(total_frames) + "," + str(reverses) + "," + str(
                prob_of_rev) + "," + str(tumbles) + "," + str(prob_of_tumble) + "," + str(
